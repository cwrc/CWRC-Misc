...
# Purpose: Shell script to normalize, join, and format and indent Orlando files using xmlformat
...
# the document (format and indent).  The formatting and indenting is performed by the Perl
# program xmlformat, written by Paul DuBois.  The Orlando files to be batch processed are put
# in a directory called "old", and the processed files are written with the same name to a
# directory called "new"; the directory "new" is created before the script is run, and it is
# created at the same level as the directory "old", and the shell script "xmlformat.sh", Perl
# program file "xmlformat.pl", and configuration file "xmlformat.conf" are placed in the parent
# directory of the "old" and "new" directories.  To run the script, type the following at the
# command prompt:

#     sh xmlformat.sh

# [The Perl program xmlformat.pl and accompanying documentation can be found at this URL:
