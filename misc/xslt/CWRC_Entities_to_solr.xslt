...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:foxml="info:fedora/fedora-system:def/foxml#" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mods="http://www.loc.gov/mods/v3" exclude-result-prefixes="mods">
 ...
 -->
    ...
    <xsl:template match="foxml:datastream[@ID='PLACE']/foxml:datastreamVersion[last()]" name="index_CWRC_PLACE_ENTITY">
        ...
        <xsl:variable name="identity" select="$content/entity/place/identity"></xsl:variable>
        <xsl:variable name="description" select="$content/entity/place/description"></xsl:variable>
    ...
    <xsl:template match="foxml:datastream[@ID='ORGANIZATION']/foxml:datastreamVersion[last()]" name="index_CWRC_ORGANIZATION_ENTITY">
        ...
        <xsl:variable name="identity" select="$content/entity/organization/identity"></xsl:variable>
        <xsl:variable name="description" select="$content/entity/organization/description"></xsl:variable>
    ...
    <xsl:template match="foxml:datastream[@ID='TITLE']/foxml:datastreamVersion[last()]" name="index_CWRC_TITLE_ENTITY">
        ...
        <!-- 'edged' (edge n-gram) text, for auto-completion -->
        <xsl:variable name="identity" select="$content/mods:mods"></xsl:variable>
        ...
        <xsl:choose>
            <xsl:when test="mods:originInfo/mods:issuance/text()='monographic'">
                ...
                <xsl:call-template name="assemble_cwrc_title_formats">
                    <xsl:with-param name="prefix" select="$local_prefix"></xsl:with-param>
                    <xsl:with-param name="date" select="mods:originInfo/mods:dateIssued/text()"></xsl:with-param>
                    <xsl:with-param name="format" select="'Monograph whole'"></xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <!-- monograph chapter -->
            <xsl:when test="mods:relatedItem/mods:originInfo/mods:issuance/text()='monographic'">
                <xsl:call-template name="assemble_cwrc_title_formats">
                    <xsl:with-param name="prefix" select="$local_prefix"></xsl:with-param>
                    <xsl:with-param name="date" select="mods:relatedItem/mods:originInfo/mods:dateIssued/text()"></xsl:with-param>
                    <xsl:with-param name="format" select="'Monograph chapter'"></xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <!-- periodical article -->
            <xsl:when test="mods:relatedItem/mods:originInfo/mods:issuance/text()='continuing'">
                <xsl:call-template name="assemble_cwrc_title_formats">
                    <xsl:with-param name="prefix" select="$local_prefix"></xsl:with-param>
                    <xsl:with-param name="date" select="mods:relatedItem/mods:part/mods:date/text()"></xsl:with-param>
                    <xsl:with-param name="format" select="'Periodical article'"></xsl:with-param>
                </xsl:call-template>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="foxml:datastream[@ID='PERSON']/foxml:datastreamVersion[last()]" name="index_CWRC_PERSON_ENTITY">
        ...
        <xsl:variable name="identity" select="$content/entity/person/identity"></xsl:variable>
        <xsl:variable name="description" select="$content/entity/person/description"></xsl:variable>
    ...
    <xsl:template match="mods:titleInfo">
        ...
        <xsl:variable name="title_type">
            <xsl:call-template name="get_title_type"></xsl:call-template>
        </xsl:variable>

        <field>
            <xsl:attribute name="name">
                <xsl:value-of select="concat($prefix, $title_type, $suffix)"></xsl:value-of>
            </xsl:attribute>

            <xsl:value-of select="mods:title/text()"></xsl:value-of>
    ...
    <xsl:template match="mods:name">
        ...
        <field>
            <xsl:attribute name="name">
                <xsl:value-of select="concat($prefix, 'author', $suffix)"></xsl:value-of>
            </xsl:attribute>

            <xsl:call-template name="assemble_cwrc_title_author"></xsl:call-template>
        </field>
    ...
    <xsl:template name="get_title_type">
        <xsl:choose>
            <xsl:when test="not(@type) and mods:title">
                <xsl:value-of select="'uniformTitle'"></xsl:value-of>
            </xsl:when>
            ...
            <xsl:when test="@type='alternative' or @type='abbreviated' or @type='translated' or @type='uniform'">
                <xsl:value-of select="'variantTitle'"></xsl:value-of>
    ...
    <xsl:template name="assemble_cwrc_title_author">
        ...
        <!-- does a surname exist -->
        <xsl:variable name="is_surname_present" select="mods:namePart/@type='family'"></xsl:variable>
        <!-- does a forename exist -->
        <xsl:variable name="is_forename_present" select="mods:namePart/@type='given'"></xsl:variable>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$is_surname_present or $is_forename_present">
                <xsl:if test="$is_forename_present">
                    <xsl:value-of select="normalize-space(mods:namePart[@type='given']/text())"></xsl:value-of>
                </xsl:if>
                <xsl:if test="$is_forename_present and $is_surname_present">
                    <xsl:text> </xsl:text>
                </xsl:if>
                <xsl:if test="$is_surname_present">
                    <xsl:value-of select="normalize-space(mods:namePart[@type='family']/text())"></xsl:value-of>
                </xsl:if>
            </xsl:when>
            <!-- namePart -->
            <xsl:when test="mods:namePart">
                <xsl:value-of select="normalize-space(mods:namePart)"></xsl:value-of>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text></xsl:text>
            </xsl:otherwise>
    ...
    <xsl:template name="assemble_cwrc_title_formats">
        ...
        <field>
            <xsl:attribute name="name">
                <xsl:value-of select="concat($prefix, 'date', '_dt')"></xsl:value-of>
            </xsl:attribute>
        ...
        <field>
            <xsl:attribute name="name">
                <xsl:value-of select="concat($prefix, 'format', '_s')"></xsl:value-of>
            </xsl:attribute>
