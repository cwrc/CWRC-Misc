...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:foxml="info:fedora/fedora-system:def/foxml#" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mods="http://www.loc.gov/mods/v3" exclude-result-prefixes="mods">
    ...
    <!-- sfdg  -->
    <xsl:template match="/">
        <xsl:apply-templates select="/foxml:datastream/foxml:datastreamVersion"/>
        <!-- 
    ...
    <xsl:template match="foxml:datastream[@ID='TITLE']/foxml:datastreamVersion[last()]" name="index_CWRC_TITLE_ENTITY">
        ...
        <!-- ensure that the preferred/authoritative title (name of title) is first -->
        ...
        <xsl:for-each select="$identity/mods:titleInfo">
            <field>
                <xsl:attribute name="name">
                    <xsl:value-of select="concat($prefix, 'variant_title', '_ms')"/>
                </xsl:attribute>
    ...
    <xsl:template match="mods:titleInfo">
        ...
        <field>
            <xsl:attribute name="name">
                <xsl:value-of select="concat($prefix, 'uniformTitle', $suffix)"/>
            </xsl:attribute>

            <xsl:call-template name="assemble_cwrc_preferred_title"/>
        </field>
    ...
    <xsl:template name="assemble_cwrc_preferred_title">
        <xsl:choose>
            ...
            <xsl:when test="@type='alternative' or @type='abbreviated' or @type='translated' or @type='uniform'">
                <!-- multiple titles, don't use type='alternative' --> </xsl:when>
    ...
    <xsl:template name="assemble_cwrc_title_author">
        <!-- does a surname exist -->
        <xsl:variable name="is_surname_present" select="namePart/@type='family'"/>
        <!-- does a forename exist -->
        <xsl:variable name="is_forename_present" select="namePart/@type='given'"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$is_surname_present or $is_forename_present">
                <xsl:if test="$is_forename_present">
                    <xsl:value-of select="normalize-space(namePart[@type='given']/text())"/>
                </xsl:if>
                <xsl:if test="$is_forename_present and $is_surname_present">
                    <xsl:text> </xsl:text>
                </xsl:if>
                <xsl:if test="$is_surname_present">
                    <xsl:value-of select="normalize-space(namePart[@type='family']/text())"/>
                </xsl:if>
            </xsl:when>
            <!-- namePart -->
            <xsl:when test="namePart">
